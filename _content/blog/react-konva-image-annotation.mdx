...
In line 15, by using `useMemo` we prevented the image element from being created repeatedly in each render. Whatever Image's size is, we set it to Satge's width and height. We did this in the image onload event listener. We removed the event listener here with `useEffect` return.
