---
title: 'Calculate Number of Characters Fit into an HTML Element With Custom React Hook'
date: '2022-03-05'
lastmod: '2022-03-05'
tags: ['react', 'react-hooks', 'problem-solving']
draft: false
...
The challenge is that the width of the buttons were not fixed, and the text inside of them were not fixed as well. Actually this information comes from an endpoint. In short, I had to do the following: calculating the width of the button, then calculating the width of the button text width; if the width of the text is larger, then cut the text in the middle with three dots. But I needed to find how many characters should be shown? Let's solve this together.
...
In my solution, I used [`ResizeObserver`](https://developer.mozilla.org/en-US/docs/Web/API/ResizeObserver#examples) which is a Web API Interface to calculate element's width, and it is available in most browsers. If it is not available, there are some polyfills libraries.

You can can calculate an element's size with resize event as well, however, I suggest to use ResizeObserver instead, because it is more reliable since it is more easy to disconnect the observer and stop the listening callbacks, in react world, it means it is more easy to clean the effect. Actually I suggest you to read [why ResizeObserver is needed?](https://github.com/WICG/resize-observer/blob/master/explainer.md) article.
I am not going to go in detail for this part, but you can find the full code from my example repository in [useElementWidth.js](https://github.com/definite2/truncate-middle/blob/main/src/hooks/useElementWidth.js) file.
